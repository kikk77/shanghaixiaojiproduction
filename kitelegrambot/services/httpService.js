...
async function handleChannelApiRequest(pathname, method, data) {
    try {
        ...
        if (endpoint === 'configs' && pathParts[5]) {
            ...
            if (action === 'scan-history' && method === 'POST') {
                // 扫描并克隆历史消息
                const {
                    startMessageId = 1,
                    endMessageId = null,
                ...
                // 异步执行扫描，避免长时间等待
                const scanPromise = configService.scanAndCloneHistory(
                    id, 
                    bs.getBotInstance(), 
                    {
                        startMessageId,
                ...
                // 存储扫描任务状态
                if (!global.historyScanTasks) {
                    global.historyScanTasks = new Map();
                }

                const taskId = `${id}_${Date.now()}`;
                global.historyScanTasks.set(id, {
                    taskId,
                    status: 'running',
                    startTime: Date.now(),
                    progress: {
                ...
                scanPromise.then(result => {
                    const task = global.historyScanTasks.get(id);
                    if (task) {
                        task.status = result.success ? 'completed' : 'failed';
                        task.endTime = Date.now();
                        task.result = result;
                        if (result.success) {
                            task.progress = result.data;
                        }
                    }
                }).catch(error => {
                    const task = global.historyScanTasks.get(id);
                ...
                return {
                    success: true,
                    message: '历史消息扫描已开始，请使用 scan-status 查看进度',
                    taskId
                };
            }

            if (action === 'scan-status' && method === 'GET') {
                // 获取历史扫描状态
                if (!global.historyScanTasks) {
                    return {
                        success: true,
